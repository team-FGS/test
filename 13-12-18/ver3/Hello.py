...
def decodeCmd( cmd):
    ...
    if device == '1':
        ...
        if _status == '1':
            dev.status = 'ON'
            dev.temperature = str(value1) + '.' + str(dec1)
        else:
            dev.status = 'OFF'
        ...
        if _status == '1':
            dev.status = 'ON'
            dev.temperature = str(value2) + '.' + str(dec2)
            if value1 == '1':
        ...
        if _status == '1':
            dev.status = 'ON')
            dev.channel = str(value1)
            dev.sound_volume = str(value2)
        else:
            dev.status = 'OFF'
            dev.channel = 'OFF'
        ...
        if status == '1':
            dev.status = 'ON'
            dev.channel = str(value1) + '.' = str(dec1)
            dev.sound_volume = str(value2)
        else:
            dev.status = 'OFF'
            dev.channel = 'OFF'
